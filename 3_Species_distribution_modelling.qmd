...
This code chuck imports the occurrence data obtained using the `2_Occurrences` Quarto notebook and sets up parameters for use later on in the script. It also creates a unique taxon list from the occurrence data for creating folder names to place results and for running the loops on each taxon.
